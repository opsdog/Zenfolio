...
int main(int argc, char *argv[]) {
...
#ifdef DEBUG
  printf("GetAllGalleriesXML:  entry - %d\n",argc);
#endif

  /* arg checking */

#ifdef DEBUG
  printf("GetAllGalleriesXML:  arg1 - %s\n",argv[1]);
#endif
...
#ifdef DEBUG
  printf("GetAllGalleriesXML:  AuthToken copied\n");
#endif

  /*  open the temp file to dump the reply into */

  tempfile = fopen("/tmp/dougee-XMLAllGalleries", "w");

#ifdef DEBUG
  printf("GetAllGalleriesXML:  tempfile open\n");
#endif
  ...
  strcpy(Content,"loginName=opsdog");
  ...
  /*  strcpy(Header,"POST /zf/api/zfapi.asmx/LoadGroupHierarchy HTTP/1.1\n");*/
  strcpy(Header,"POST /api/1.8/zfapi.asmx/LoadGroupHierarchy HTTP/1.1\n");
  ...
  BytesWritten = SSL_write(ZenSSL, Header, HeaderLength);
#ifdef DEBUG
  printf("Header written - %d\n", BytesWritten);
#endif
  BytesWritten = SSL_write(ZenSSL, Content, ContentLength);
#ifdef DEBUG
  ...
  */
  ...
  Tempfile = open("/tmp/dougee-XMLAllGalleries", O_RDONLY | O_CREAT);
  XMLfile = open("XML-AllGalleries.xml", O_WRONLY | O_CREAT);
